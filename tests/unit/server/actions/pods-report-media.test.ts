...
describe("reportMedia — dedup + auto-hide (LAC-2914, LAC-2897 H4/M3)", () => {
	...
	it("enforces the per-user report rate limit", async () => {
		// reportMedia guards with a real module-private sliding-window limiter
		// (REPORT_MAX_PER_WINDOW = 20/hour/user, in-memory). Drive the same
		// reporter past the ceiling and assert the surfaced rejection. Looping
		// past 20 keeps this robust to the handful of attempts earlier tests in
		// this file already charged to reporter-1's window.
		let rejected: Error | null = null;
		for (let i = 0; i < 25 && !rejected; i++) {
			try {
				await reportMedia("media-1", "spam");
			} catch (err) {
				if (/too many reports/i.test((err as Error).message)) {
